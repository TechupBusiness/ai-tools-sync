...
function detectContentType(
  ...
  if (fmResult.ok && !fmResult.value.isEmpty) {
    ...
    if (data.globs !== undefined || data.always_apply !== undefined || data.alwaysApply !== undefined) {
  ...
  if (lowerPath.includes('persona') || lowerPath.includes('agent') || lowerPath.includes('droid')) return 'persona';
...
function computeDiff(existing: string, incoming: string): DiffDetails {
  ...
  const contentChanged = existingFm.ok && incomingFm.ok
...
async function compareWithExisting(
  ...
  const targetExists = await fs.stat(inputFile.targetPath)
    .then(() => true)
    .catch(() => false);
  ...
  inputFile.diffDetails = computeDiff(
...
function printMergeReport(files: InputFile[], verbose?: boolean): void {
  ...
  if (byStatus.new.length > 0) {
    ...
    for (const f of byStatus.new) {
      const targetRelPath = path.relative(
        path.dirname(path.dirname(f.targetPath)),
        f.targetPath
      );
  ...
  for (const file of mergeable) {
    ...
    if (!options.yes && file.status === 'modified') {
      const action = await prompt(
        `Merge ${file.relativePath}? (y)es, (n)o, (d)iff`,
        'y'
      );
