...
- [ ] Root gitignore only contains `CLAUDE.md`, `AGENTS.md`, `mcp.json`, manifest (new default)
- [ ] No root files generated → Root gitignore only has manifest file
- [ ] Manifest is null → Use default root-only paths
- [ ] Existing managed section in root gitignore → Replace with new (smaller) content
- [ ] `mcp.json` not generated (no MCP config) → Don't include in paths
- [ ] Only some root files generated → Only include those present in manifest
...
- [ ] `getDefaultGitignorePaths()` returns only root-level files (no tool folders)
- [ ] `getGitignorePaths()` returns only root-level files from manifest (no tool folders/directories)
- [ ] Root `.gitignore` only contains: `CLAUDE.md`, `AGENTS.md`, `mcp.json`, `.ai-tool-sync-generated`
- [ ] Tool folder paths are NOT in root gitignore (managed by T223 per-folder gitignores)
- [ ] All tests pass (`npm test -- gitignore.test.ts manifest.test.ts`)
...
1. [ ] Update `getDefaultGitignorePaths()` in `src/utils/gitignore.ts` to return only root-level files
2. [ ] Add `DEFAULT_GENERATED_ROOT_FILES` constant in `src/utils/manifest.ts`
3. [ ] Update `getGitignorePaths()` in `src/utils/manifest.ts` to use root-only files
4. [ ] Update tests in `tests/unit/utils/gitignore.test.ts` for new behavior
5. [ ] Update tests in `tests/unit/utils/manifest.test.ts` for new behavior
6. [ ] Run `npm test -- gitignore.test.ts manifest.test.ts`
7. [ ] Run `npm run lint --fix`
8. [ ] Run `npm run typecheck`
9. [ ] Update plan.md with T224 completion status
