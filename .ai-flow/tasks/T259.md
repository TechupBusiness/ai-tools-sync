...
- [ ] Use snake_case for frontmatter fields (`always_apply`, not `alwaysApply`)
- [ ] Include `version: 1.0.0` for tracking
- [ ] Set `targets: [cursor, claude, factory]` to apply across all platforms
- [ ] Use `globs:` patterns that trigger when working with config files
- [ ] Set `category: documentation` or `tooling`
- [ ] Set `priority: medium`
...
- [ ] Content must be comprehensive but scannable (use tables, code blocks)
- [ ] Include both beginner and advanced patterns
- [ ] Reference but don't duplicate `docs/CONFIGURATION.md` content
- [ ] Keep examples minimal and focused on patterns, not full implementations
...
- [ ] File parses without errors via `parseRule()`
- [ ] Frontmatter passes JSON schema validation
- [ ] File included in `ai-sync sync` output for all targets
- [ ] Glob patterns trigger correctly on `.ai-tool-sync/` files
- [ ] Content renders correctly as Cursor MDC, Claude skill, Factory skill
...
- [ ] `defaults/rules/authoring-guide.md` exists and parses without errors
- [ ] `npm run format:check` passes (or run `npm run format`)
- [ ] `ai-sync validate` succeeds
- [ ] `ai-sync --dry-run` includes the new rule in output
- [ ] Content covers all four kinds (rule, persona, command, hook)
- [ ] Tool and model mappings are documented
- [ ] Validation workflow checklist is included
- [ ] Platform extension examples are provided
- [ ] File shows up in generated outputs for cursor, claude, factory
- [ ] plan.md updated with T259 completion
