...
- [x] Use `Result<T, E>` for error handling (see `src/utils/result.ts`)
- [x] Reuse `updateGitignoreContent()`, `createManagedSection()` from existing gitignore.ts
- [x] Use same markers: `GITIGNORE_START_MARKER`, `GITIGNORE_END_MARKER`
- [x] Use `fileExists`, `readFile`, `writeFile`, `ensureDir` from `src/utils/fs.ts`
...
- [x] `updateToolFolderGitignores()` creates `.cursor/.gitignore`, `.claude/.gitignore`, `.factory/.gitignore` as needed
- [x] Each gitignore contains only relative paths within that folder (not `.cursor/rules/` but `rules/`)
- [x] Existing user content in tool folder gitignores is preserved
- [x] Folders without generated content are skipped
- [x] Dry run mode reports changes without writing
- [x] All tests pass (`npm test -- gitignore.test.ts`)
- [x] No lint errors (`npm run lint`)
- [x] Types are strict (no `any`)
- [x] Functions exported from `src/utils/index.ts`
...
1. [x] Add `ToolFolderGitignoreConfig`, `ToolFolderGitignoreResult`, `UpdateToolFolderGitignoreOptions` interfaces to `src/utils/gitignore.ts`
2. [x] Add `groupFilesByToolFolder()` helper function
3. [x] Add `updateGitignoreInFolder()` internal function (reuses existing utilities)
4. [x] Add `updateToolFolderGitignores()` main export function
5. [x] Export new function from `src/utils/index.ts`
6. [x] Integrate into `src/cli/commands/sync.ts` in `generateManifestAndGitignore()`
7. [x] Add tests to `tests/unit/utils/gitignore.test.ts`
8. [x] Run `npm test -- gitignore.test.ts`
9. [x] Run `npm run lint --fix`
10. [x] Run `npm run typecheck`
11. [x] Update plan.md with T223 completion status
