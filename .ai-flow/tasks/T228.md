...
- [x] No conditional blocks → Return content unchanged
- [x] Empty block `{{#claude}}{{/claude}}` → Remove entirely (including tags)
- [x] Nested blocks → Outer block takes precedence (inner tags treated as content)
- [x] Mismatched tags `{{#claude}}...{{/cursor}}` → Leave as-is (no match)
- [x] Invalid target in condition → Ignore that part of condition
- [x] Block at start of content → No leading newline in output
- [x] Block at end of content → No trailing newline added
- [x] Multiple blocks on same line → Process each independently
- [x] Whitespace inside tags `{{ #claude }}` → Don't match (strict syntax)
- [x] Content with `{{` but not a valid block → Leave as-is
...
- [x] `transformConditionalContent()` correctly processes all syntax variants
- [x] Single platform blocks work (`{{#claude}}...{{/claude}}`)
- [x] Negation works (`{{#!cursor}}...{{/!cursor}}`)
- [x] OR operator works (`{{#claude|factory}}...{{/claude|factory}}`)
- [x] AND with negation works (`{{#claude&!cursor}}...{{/claude&!cursor}}`)
- [x] Non-matching blocks are fully removed
- [x] Matching blocks have tags stripped, content preserved
- [x] Edge cases handled gracefully (mismatched tags, invalid targets, nested blocks)
- [x] All tests pass (`npm test -- conditional-content.test.ts`)
- [x] No lint errors (`npm run lint`)
- [x] Types are strict (no `any`)
- [x] Exported from `src/transformers/index.ts`
...
1. [x] Create `src/transformers/conditional-content.ts` with `transformConditionalContent()` function
2. [x] Implement `parseCondition()` helper for parsing condition strings
3. [x] Implement `evaluateCondition()` helper for matching against target
4. [x] Handle all operators: single, negation (!), OR (|), AND (&)
5. [x] Add whitespace cleanup logic
6. [x] Export from `src/transformers/index.ts`
7. [x] Create `tests/unit/transformers/conditional-content.test.ts`
8. [x] Add tests for all syntax variants
9. [x] Add tests for edge cases
10. [x] Run `npm test -- conditional-content.test.ts`
11. [x] Run `npm run lint --fix`
12. [x] Run `npm run typecheck`
13. [x] Update plan.md with T228 completion status
