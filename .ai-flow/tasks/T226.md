...
- [x] Follow `exactOptionalPropertyTypes` - conditionally include keys (see learnings.md T159)
...
- [x] Empty `when:` field → Include rule (no condition)
- [x] Missing `when:` field → Include rule (no condition)
- [x] Unknown namespace → Return `UNKNOWN_NAMESPACE` error
- [x] Missing colon in identifier → Return `PARSE_ERROR`
- [x] Scoped npm packages (`npm:@scope/pkg`) → Handle @ in name
- [x] Go module paths with slashes → Handle full path as name
- [x] File paths with spaces → Support quoted paths
- [x] Ecosystem file missing (no package.json) → Dependency check returns false
- [x] Multiple dependency files (requirements.txt + pyproject.toml) → Merge results
- [x] Negation with comparison (`!pkg:type == "module"`) → Apply negation to result
- [x] Case sensitivity → Package names case-sensitive, namespaces lowercase
- [x] Whitespace in expressions → Trim and normalize
- [x] Chained conditions with precedence → AND before OR
- [x] Parse errors → Return descriptive error, don't crash
...
- [x] Parse namespaced identifiers (`npm:react`, `pip:django`, etc.)
- [x] Support all 10 package ecosystems (npm, pip, go, cargo, composer, gem, pub, maven, gradle, nuget)
- [x] Support `file:` and `dir:` checks
- [x] Support `pkg:` for package.json field access
- [x] Support `var:` for user-defined variables
- [x] Support negation with `!` prefix
- [x] Support `==` and `!=` operators for comparisons
- [x] Support `&&` and `||` logical operators
- [x] Cache dependency lookups per ecosystem
- [x] All tests pass (`npm test -- condition-evaluator.test.ts`)
- [x] No lint errors (`npm run lint`)
- [x] Types are strict (no `any`)
- [x] Exports added to `src/transformers/index.ts`
- [x] `when` field added to Rule interface
...
1. [x] Create `src/transformers/condition-evaluator.ts` with interfaces
2. [x] Implement `parseIdentifier()` function
3. [x] Implement `parseValue()` helper function
4. [x] Implement `parseConditionTerm()` function
5. [x] Implement `parseConditionExpression()` for compound conditions
6. [x] Implement `loadNpmDependencies()` function
7. [x] Implement `loadPipDependencies()` function
8. [x] Implement dependency loaders for other ecosystems (stub implementations OK for MVP)
9. [x] Implement `checkDependency()` with caching
10. [x] Implement `evaluateSingleCondition()` function
11. [x] Implement `evaluateConditionExpression()` main export
12. [x] Implement `shouldIncludeRule()` helper export
13. [x] Implement `buildProjectContext()` factory function
14. [x] Export from `src/transformers/index.ts`
15. [x] Add `when?: string` field to `Rule` interface in `src/parsers/rule.ts`
16. [x] Create test fixtures in `tests/fixtures/conditional-rules/`
17. [x] Add tests to `tests/unit/transformers/condition-evaluator.test.ts`
18. [x] Run `npm test -- condition-evaluator.test.ts`
19. [x] Run `npm run lint --fix`
20. [x] Run `npm run typecheck`
21. [x] Update plan.md with T226 completion status
