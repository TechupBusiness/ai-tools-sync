...
export interface ConvertOptions {
  ...
  runLint?: boolean;             // invoke rule lint provided by T231
}
...
- [x] Use `Result<T, E>` (`src/utils/result.ts`); prefer returning `ConvertError` instead of throwing.
- [x] Keep generic frontmatter in snake_case; platform-only fields go under `cursor`/`claude`/`factory` extension blocks.
- [x] Reuse tool/model mappings (add reverse mapping helpers) instead of hardcoding strings.
- [x] Avoid `any`; respect `exactOptionalPropertyTypes` (conditionally include optional props).
- [x] Import via `@/` aliases and keep `import/order`.
- [x] Export only through `src/converters/index.ts`; CLI command re-exported from `src/cli/commands/index.ts`.
- [x] Preserve unknown/extra platform fields in `...extension` objects rather than dropping silently (warn when skipped).
...
- [x] No frontmatter → generate minimal frontmatter with `name`, `targets`, warn.
- [x] Comma- or newline-separated globs in Cursor files → normalize to string[].
- [x] `alwaysApply` camelCase in Cursor → map to `always_apply` boolean.
- [x] Mixed/ambiguous files (multiple H1/H2) → flag `shouldSplit`, copy to `input/` with warning.
- [x] Unknown or unmapped tools/models → preserve raw value under extension, warn.
- [x] Duplicate `name` across converted files → warn and suffix filenames, but keep original in frontmatter.
- [x] Factory droid `reasoningEffort` not in generic schema → move to `factory.reasoningEffort`.
- [x] Hook/command variable blocks (`variables`, `env`) → keep under extension if not in generic schema.
- [x] Already-generic files in `.ai-tool-sync/` passed in by mistake → detect and skip with info message.
- [x] Invalid YAML/frontmatter → surface parse errors with line info; do not drop content.
- [x] `targets` missing or platform-specific → default to `DEFAULT_TARGETS`.
...
=======
- [x] Cursor rule with `alwaysApply: true` + comma globs → outputs `always_apply: true` + string[] globs.
- [x] Cursor command with `allowedTools` → preserved in `cursor.allowedTools`; generic parse passes.
- [x] Claude agent with `tools/model` → tools/model reverse-mapped to generic; extras kept in `claude` extension.
- [x] Factory droid with `reasoningEffort` + `allowed-tools` → mapped to generic persona + `factory` extension.
- [x] CLAUDE.md / AGENTS.md with multiple sections → flagged `mixed`, copied to `input/`, warning emitted.
- [x] Unknown tool/model values → warning, still converted with raw value under extension.
- [x] Missing frontmatter → name inferred from filename, description empty, parse succeeds.
- [x] Duplicate names across converted files → warning + filename suffix to avoid overwrite.
- [x] CLI flag `--auto-convert` (or `convert` command) writes to `rules/` etc. and exits non-zero on errors in strict mode.
>>>>>>> Incoming (Background Agent changes)
- [ ] Conversion respects `runLint` (mock lint result) using T231 lint.
...
- [x] Conversion API returns `Result` with normalized generic frontmatter and passes parsers.
- [x] Reverse tool/model mapping implemented and covered by tests.
- [x] CLI (`ai-sync convert` or `migrate --auto-convert`) writes converted files into `.ai-tool-sync/{rules,personas,commands,hooks}` with warnings for skipped/mixed files.
- [x] Strict mode exits non-zero on errors (or lint failures when enabled).
- [x] No `any`, imports use `@/` alias, optional props gated.
- [x] Tests added under `tests/unit/converters/` and CLI coverage updated.
- [x] `npm run format:check`, `npm run lint`, and `npm test -- converters` pass.
- [x] plan.md updated with T238 status once implemented.

## 9. Out of Scope
- Auto-splitting mixed files into multiple generic files (only detect and warn).
- Auto-fix of lint issues (still out of scope despite T231 availability).
