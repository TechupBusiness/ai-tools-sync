...
- [ ] Use `readFile()`, `resolvePath()` from `src/utils/fs.ts`
...
- [ ] No includes in content → Return original content, `hasIncludes: false`
- [ ] Self-include (`@include ./self.md`) → Return `CIRCULAR_INCLUDE` error
- [ ] Direct circular include (A→B→A) → Return `CIRCULAR_INCLUDE` error with chain
- [ ] Indirect circular (A→B→C→A) → Return `CIRCULAR_INCLUDE` error with full chain
- [ ] Include file not found → Return `FILE_NOT_FOUND` error with path
- [ ] Include file read error (permissions) → Return `READ_ERROR` error
- [ ] Include depth exceeded → Return `MAX_DEPTH_EXCEEDED` error
- [ ] Include in frontmatter (should not match) → Ignored, pattern only matches body
- [ ] Multiple includes in same file → All resolved in order
- [ ] Nested includes (A→B→C, no cycle) → All resolved recursively
- [ ] Sibling includes (A includes B and C separately) → Both resolved
- [ ] Empty include file → Replace with empty string
- [ ] Included file has frontmatter → Frontmatter stripped, only body included
- [ ] Whitespace before `@include` → Valid, whitespace preserved in pattern
- [ ] Path with spaces (`@include path with spaces.md`) → Supported
- [ ] Absolute paths → Supported but discouraged (for portability)
...
- [ ] `resolveIncludes()` resolves `@include` directives in markdown content
- [ ] Relative paths are resolved from the including file's location
- [ ] Circular includes are detected and return descriptive error with chain
- [ ] Nested includes are supported up to configurable max depth
- [ ] Frontmatter is stripped from included files
- [ ] All tests pass (`npm test -- include-resolver.test.ts`)
...
1. [ ] Create `src/transformers/include-resolver.ts` with interfaces
2. [ ] Implement `findIncludes()` helper function
3. [ ] Implement `extractBodyContent()` helper function
4. [ ] Implement `resolveIncludePath()` helper function
5. [ ] Implement `resolveIncludesRecursive()` internal function
6. [ ] Implement `resolveIncludes()` main export function
7. [ ] Export from `src/transformers/index.ts`
8. [ ] Create test fixtures in `tests/fixtures/includes/`
9. [ ] Add tests to `tests/unit/transformers/include-resolver.test.ts`
10. [ ] Run `npm test -- include-resolver.test.ts`
11. [ ] Run `npm run lint --fix`
12. [ ] Run `npm run typecheck`
13. [ ] Update plan.md with T225 completion status
