...
- [ ] No rules found → Return success with 0 files linted
- [ ] Rule file fails to parse → Include error in results, continue linting others
- [ ] Circular requires references → Detect and report as error
- [ ] Missing frontmatter entirely → Caught by parser, not linter
- [ ] Empty globs array vs undefined globs → Both trigger "unreachable" warning
- [ ] Self-referential requires (rule requires itself) → Report as error
- [ ] Unknown lint rule ID in `--rules` option → Warn and skip
- [ ] All rules ignored via `--ignore` → Warn user, return empty results
...
1. [ ] Create `src/linters/types.ts` with all type definitions
2. [ ] Create `src/linters/rules/deprecated-fields.ts`
3. [ ] Create `src/linters/rules/best-practices.ts`
4. [ ] Create `src/linters/rules/index.ts` with `getAllLintRules()`
5. [ ] Create `src/linters/rule-linter.ts` with `lintRules()` function
6. [ ] Create `src/linters/index.ts` with public exports
7. [ ] Create `src/cli/commands/lint.ts` command
8. [ ] Register command in `src/cli/index.ts`
9. [ ] Add tests in `tests/unit/linters/`
10. [ ] Run `npm test -- rule-linter`
11. [ ] Run `npm run lint --fix`
12. [ ] Run `npm run typecheck`
13. [ ] Update plan.md with T231 completion status
