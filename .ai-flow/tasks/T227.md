...
- [x] `extends` field is undefined → Return persona unchanged
- [x] `extends` field is empty string → Validation error in parser
- [x] Parent persona not found → Warning + return child unchanged
- [x] Circular reference (A extends B, B extends A) → Error
- [x] Self-reference (A extends A) → Error
- [x] Deep inheritance (A extends B extends C) → Resolve recursively
- [x] Max depth exceeded → Error with clear message
- [x] Duplicate persona names → Error before resolution
- [x] Parent has no content, child has content → Use child content only
- [x] Parent has content, child has no content → Use parent content only
- [x] Both have content → Concatenate with separator
- [x] Platform extensions partial overlap → Merge with child priority
...
- [x] `parsePersona()` accepts and validates `extends` field
- [x] `resolvePersonaInheritance()` resolves inheritance for all personas
- [x] Child frontmatter overrides parent frontmatter
- [x] Content is concatenated with parent first, separator, then child
- [x] Platform extensions (`cursor`, `claude`, `factory`) are deep merged
- [x] Circular references are detected and reported as errors
- [x] Missing parent results in warning (not error), persona unchanged
- [x] Max depth limit prevents infinite loops
- [x] All tests pass (`npm test -- persona.test.ts`)
- [x] No lint errors (`npm run lint`)
- [x] Types are strict (no `any`)
- [x] Functions exported from `src/parsers/index.ts`
- [x] Integration added to `src/cli/commands/sync.ts`
...
1. [x] Add `extends?: string` to `Persona` interface in `src/parsers/persona.ts`
2. [x] Add `extends` validation to `validatePersonaFields()`
3. [x] Add `InheritanceWarning` type
4. [x] Add `ResolvePersonaInheritanceOptions` type
5. [x] Add `ResolvePersonaInheritanceResult` type
6. [x] Implement `mergePersonas()` helper function
7. [x] Implement `resolveInheritanceChain()` helper function
8. [x] Implement `resolvePersonaInheritance()` main function
9. [x] Export new types and functions from `src/parsers/index.ts`
10. [x] Integrate into `src/cli/commands/sync.ts` after content loading
11. [x] Add tests to `tests/unit/parsers/persona.test.ts`
12. [x] Run `npm test -- persona.test.ts`
13. [x] Run `npm run lint --fix`
14. [x] Run `npm run typecheck`
15. [x] Update plan.md with T227 completion status
