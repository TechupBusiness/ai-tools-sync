...
- [ ] `prepublishOnly` still runs tests with the memory flag applied.  
- [ ] Passing extra Vitest args (e.g., `npm test -- tests/foo.test.ts`) still works.
...
- [ ] `package.json` `test` script is Windows-compatible and applies `--max-old-space-size=4096`.  
- [ ] Windows CI matrix job passes without `NODE_OPTIONS` errors.  
- [ ] `prepublishOnly` continues to work.  
- [ ] No lint/type errors introduced; new code respects project patterns.  
- [ ] `plan.md` entry for T255 reflects the change if status updates are needed.
