...
- [x] Files annotated with `// prettier-ignore` remain unchanged.
- [x] Mixed tabs/spaces and trailing comma/semicolon inconsistencies are normalized by Prettier, not by manual edits.
- [x] Inline comments and disable directives stay attached to the code they describe after formatting.
- [x] Any `.ts` fixtures under `tests/fixtures` (if encountered) keep their intended fixture semantics after formatting.
...
- [x] `npm run format` completes with no remaining diffs.
- [x] `npm run format:check` passes after formatting.
- [x] `npm run lint` remains clean (import/order, unused vars).
- [x] Manual spot-check on a couple of CLI and transformer files to verify no behavioral edits slipped in.
...
- [x] Only formatting diffs are present; no runtime or type signature changes.
- [x] `npm run format:check` passes locally and is expected to pass in CI.
- [x] Plan entry `T254` in `plan.md` stays current (already added).
- [x] No generated or ignored assets (`dist/`, `coverage/`, docs) are modified.
