...
- [ ] Pre-release tags containing `-` publish with `next` dist-tag and mark GitHub prerelease.  
- [ ] Missing or malformed tag (non-semver) → fail with clear message, skip publish.  
- [ ] Tag/package version mismatch → fail before publish/release creation.  
- [ ] Rerun-on-failure should not republish if the version already exists on npm; add `npm view $PKG_VERSION` check and short-circuit if published.  
- [ ] Secrets absent (`NPM_TOKEN`) → fail fast with actionable error.  
- [ ] Preserve changelog generation when no previous tag (first release).  
- [ ] Do not leak secrets/logs; mask token, avoid verbose npm output flags that print auth.
...
- [ ] `.github/workflows/release.yml` triggers on `v*` tags and optional manual runs.  
- [ ] Workflow runs build + tests before any publish/release steps.  
- [ ] npm publish uses correct dist-tag (`latest` stable, `next` prerelease) and provenance.  
- [ ] GitHub release created with generated notes and attached `npm pack` artifact.  
- [ ] Tag/package version validation enforced; fail-fast on mismatch or missing secrets.  
- [ ] No duplicate publish when rerun on the same tag; idempotency guard present.  
- [ ] plan.md updated to reflect T244 spec/implementation status.
