...
- [x] Keep `exports`/`main`/`module`/`types` aligned with `tsup.config.ts` outputs (ESM + CJS + dts) to avoid runtime resolution errors.
- [x] Use the `files` allowlist (not `.npmignore`) and ensure it includes `dist`, `bin`, `defaults`, `targets`, and any other runtime assets; exclude tests, coverage, fixtures by omission.
- [x] Preserve `engines.node >=18` to match `tsup` target and README badge.
- [x] When adding optional fields (e.g., `funding`, `publishConfig`), omit keys instead of setting `undefined` to satisfy `exactOptionalPropertyTypes` (see learnings T159/T160).
- [x] Update `repository`, `bugs`, and `homepage` to the canonical GitHub repo and README anchor so npm links are correct.
- [x] Leave CLI entry (`bin/ai-sync.js`) pointing at built output; do not introduce TS/tsx-only dependencies into runtime paths.
...
- [x] `npm pack --dry-run` should NOT include `coverage/`, `tests/`, `src/`, `.ai-flow/`, `.github/`, or vitest snapshots/fixtures.
- [x] Tarball install (`npm install ./ai-tool-sync-*.tgz`) must run `ai-sync --help` without devDependencies or TypeScript available.
- [x] `license` and `README.md` should be included by default; ensure nothing in `files` or future `.npmignore` would exclude them.
- [x] Version bump workflow should keep `prepublishOnly` reproducible (build + tests) and not depend on local paths.
...
- [x] `npm run build` produces `dist/` with both ESM and CJS + dts.
- [x] `npm test` still passes after metadata changes.
- [x] `npm pack --dry-run` lists only expected files (`dist`, `bin`, `defaults`, `targets`, `LICENSE`, `README`, `package.json`).
- [x] Install from packed tarball in a clean temp dir and run `npx ai-sync --version` (or `ai-sync --help`) to confirm bin works.
- [x] Spot-check metadata links from `npm view <package> repository.url bugs.url homepage` (or local package.json) for correctness.
...
- [x] `package.json` metadata is accurate (name, description, keywords, repository, bugs, homepage, license, engines) and includes `funding` if applicable.
- [x] `files` allowlist includes all runtime assets (dist, bin, defaults, targets) and excludes tests/coverage/fixtures by omission.
- [x] `publishConfig` explicitly set for public npm publish with correct registry (unless already correct).
- [x] `npm pack --dry-run` output matches expected file list; tarball install runs `ai-sync --help` successfully.
- [x] No lint/typecheck regressions from metadata/script changes.
- [x] plan.md updated to reflect T243 completion status when done.
